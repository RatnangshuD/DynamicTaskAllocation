...
model.agent(1).flame_radius=10*(model.agent(1).all_s/pi).^0.5;
...
for t=0:model.dt:100
    if Ts_counter==0 || S_flag ==1
        ...
        for j=1:model.no_Task
           ...
           model.agent(1).flame_radius(j)=10*(model.agent(1).all_s(j)/pi)^0.5;
        end
    ...
    elseif Ts_counter==model.Ts/model.dt-1
       ...
       Ts2_counter=Ts2_counter+model.dt;
    ...
    for i=1:model.no_agent
           ...
           allocation_sum=allocation_sum+model.agent(i).away_flag;
                ...
                for i=1:model.agent(1).un_allocated
                    Id=model.agent(1).un_allocated_Id(i);
                     ...
                     if E1>E2
                         ...
                         model.agent(1).bidder_Id(count1)=Id;
                         model.agent(1).no_bidder=count1;
                ...
                if model.agent(1).no_bidder>0
                   ...
                   for i=1:model.agent(1).no_bidder
                    ...
                    if new_total_time < Present_total_time
                        ...
                        model.agent(best_bidder_Id).away_flag=1;
                        model.agent(best_bidder_Id).task_pos=model.Task(j).pos;
                        tracking_vector(best_bidder_Id,:)=model.Task(j).pos-model.agent(best_bidder_Id).pos;
                        tracking_vector(best_bidder_Id,:)=tracking_vector(best_bidder_Id,:)/norm(tracking_vector(best_bidder_Id,:));
                        F1(best_bidder_Id)=0;
